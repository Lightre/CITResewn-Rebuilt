package com.lightre.citresewnrebuilt;
...
import com.lightre.citresewnrebuilt.cit.*;
...
public class CITResewnCommand {
    /**
     * @see com.lightre.citresewnrebuilt.mixin.ChatScreenMixin
     */
    ...
    public static void register() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("citresewn").executes(context -> {
                ...
                return 1;
            }).then(literal("config").executes(context -> { //citresewn config
                openConfig = true;

                return 1;
            })).then(literal("analyze").then(literal("pack").then(argument("pack", new LoadedCITPackArgument()).executes(context -> { //citresewn analyze <pack>
                ...
                if (ActiveCITs.isActive()) {
                    ...
                    List<CITCondition> conditions = ActiveCITs.getActive().cits.values().stream().flatMap(Collection::stream).filter(cit -> cit.packName.equals(pack)).flatMap(cit -> Arrays.stream(cit.conditions)).toList();
                ...
                } else context.getSource().sendFeedback(of("Not active"));
    ...
    /**
     * Greedy string argument that is limited to cit pack names loaded in {@link com.lightre.citresewnrebuilt.cit.ActiveCITs}.
     */
    private static class LoadedCITPackArgument implements ArgumentType<String> {
        @Override
        public String parse(StringReader reader) throws CommandSyntaxException {
            StringBuilder builder = new StringBuilder();
            while (reader.canRead()) builder.append(reader.read());
